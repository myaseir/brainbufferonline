...
import { User, PlusCircle, ArrowUpRight, LogOut, Users,Clock, Wifi, AlertTriangle, RefreshCcw } from 'lucide-react';
import { useNetworkCheck } from '../../hooks/useNetworkCheck';
import OnlineStatus from './OnlineStatus'; // Ensure the path matches your file location
import { playPopSound } from '../../utils/sounds';
export default function Navbar({ user, onDeposit, onWithdraw, onLogout, onOpenFriends, onOpenHistory, requestCount, onRefresh }) {
        ...
        <div className="text-left relative">
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              onClick={() => {
    handleManualRefresh(); // 🔊 Play the sound first
    playPopSound(); // Then run the pause logic
  }}
...
<div className="flex justify-center items-center md:flex-1">
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto mt-2 md:mt-0">
        <div className="flex flex-col items-center md:items-end px-0 md:px-6 md:border-r border-slate-100 w-full md:w-auto">
         ...
         {/* Place this inside the Wallet Balance column in Navbar.jsx */}
<button 
  
          onClick={() => {
    onOpenHistory(); // 🔊 Play the sound first
    playPopSound(); // Then run the pause logic
  }}
  className="mt-2 group flex items-center gap-1.5 px-3 py-1 bg-slate-50 hover:bg-indigo-50 border border-slate-100 hover:border-indigo-100 rounded-full transition-all duration-300 shadow-sm"
>
  <Clock size={10} className="text-slate-400 group-hover:text-indigo-500 transition-colors" />
  <span className="text-[9px] font-black text-slate-500 group-hover:text-indigo-600 uppercase tracking-wider">
    History
  </span>
  <ArrowUpRight size={10} className="text-slate-300 group-hover:text-indigo-400 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
</button>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 w-full md:w-auto">
           ...
           <button   onClick={() => {
    onDeposit(); // 🔊 Play the sound first
    playPopSound(); // Then run the pause logic
  }}
          ...
          <button 
              onClick={() => {
    onWithdraw(); // 🔊 Play the sound first
    playPopSound(); // Then run the pause logic
  }}
          ...
          <button 
                   onClick={() => {
    onOpenFriends(); // 🔊 Play the sound first
    playPopSound(); // Then run the pause logic
  }}
