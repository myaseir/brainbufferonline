  ...
  return (
    <div className="min-h-screen bg-[#fcfdfd] text-slate-800 p-4 md:p-8 font-sans selection:bg-green-100">
      ...
      <div className="max-w-6xl mx-auto relative z-10 space-y-8">
        ...
        {/* --- HEADER --- */}
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white/80 backdrop-blur-xl border border-white p-6 rounded-3xl shadow-lg shadow-green-900/5">
          ...
          <div className="flex items-center gap-5 w-full md:w-auto">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight text-slate-900 uppercase truncate max-w-[200px] md:max-w-xs">
                {user?.username || 'Commander'}
              </h1>
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className="bg-green-50 text-green-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider border border-green-100 whitespace-nowrap">Rank: Elite</span>
                <span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest whitespace-nowrap">• Pakistan Server</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 w-full md:w-auto">
            ...
            {/* Wallet Container */}
            <div className="flex flex-col items-start md:items-end px-0 md:px-6 md:border-r border-slate-100 w-full md:w-auto">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-1">Wallet Balance</span>
              
              {/* Balance Amount */}
              <div className="flex items-baseline gap-2 mb-3 md:mb-0">
                <span className="text-3xl md:text-2xl font-black text-slate-900 tabular-nums break-all leading-none">
              ...
              <div className="flex gap-2 w-full md:w-auto mt-2 md:mt-1">
                <button 
                  onClick={() => setShowDepositModal(true)} 
                  className="flex-1 md:flex-none flex justify-center items-center gap-2 bg-emerald-500 text-white px-4 py-3 md:py-2 rounded-xl text-[10px] font-black uppercase shadow-md hover:bg-emerald-600 transition-all active:scale-95"
                >
                  <PlusCircle size={14}/> Deposit
                </button>
                <button 
                  onClick={() => setShowWithdrawModal(true)} 
                  className="flex-1 md:flex-none flex justify-center items-center gap-2 bg-slate-900 text-white px-4 py-3 md:py-2 rounded-xl text-[10px] font-black uppercase shadow-md hover:bg-slate-700 transition-all active:scale-95"
                >
            ...
            {/* Logout Button - Hidden on small mobile, accessible differently or kept here */}
            <button onClick={onLogout} className="p-4 bg-slate-50 hover:bg-red-50 hover:text-red-500 border border-slate-100 rounded-2xl transition-all shadow-sm hidden md:block">
                <LogOut size={24} />
            </button>
            {/* Mobile Logout (Full width button) */}
            <button onClick={onLogout} className="md:hidden flex items-center justify-center gap-2 w-full p-4 bg-slate-50 text-slate-500 hover:bg-red-50 hover:text-red-500 border border-slate-100 rounded-2xl transition-all shadow-sm font-bold text-xs uppercase tracking-widest">
