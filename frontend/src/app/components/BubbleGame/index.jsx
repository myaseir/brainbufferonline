...
// 🎨 HELPER: Professional Result Styling with High-Conversion Messaging
  const resultData = useMemo(() => {
    ...
    // 1. Victory by Abandonment (The "Rage Quit" scenario)
    if (isDisconnect && logic.won) {
      return {
        title: "Opponent Quit",
        subtitle: "100% Payout Awarded",
        icon: UserX,
        bg: "bg-emerald-50",
        text: "text-emerald-600",
        border: "border-emerald-200",
        pkr: "+200 PKR", 
        pkrColor: "text-emerald-600 font-black"
      };
    }
    ...
    if (logic.isDraw) {
      return {
        title: "Battle Draw",
        subtitle: "Stakes Fully Refunded",
        icon: MinusCircle,
        bg: "bg-slate-50", // More professional neutral gray for draws
        text: "text-slate-600",
        border: "border-slate-200",
        pkr: "REFUNDED 100",
        pkrColor: "text-slate-600 font-black"
      };
    }
    
    // 3. Normal Win (The "Winner Takes All" scenario)
    if (logic.won) {
      return {
        title: "Victory!",
        subtitle: "Full Pot Collected",
        icon: Trophy,
        bg: "bg-emerald-500", // Darker background for more impact on wins
        text: "text-white",     // White icon on emerald bg
    ...
    return {
      ...
      pkr: "STAKES LOST",
      pkrColor: "text-red-600 font-black"
    };
