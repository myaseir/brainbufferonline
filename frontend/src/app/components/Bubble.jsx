...
const scoreRef = useRef(0);
  ...
  const maxRound = 20;
// Add this function (around line 50-60, after imports/helpers)
const sendChallenge = async (targetId, username) => {
  const token = localStorage.getItem('token');
  if (socket && socket.readyState === WebSocket.OPEN) {
    // Try WebSocket first
    socket.send(JSON.stringify({ type: 'SEND_CHALLENGE', target_id: targetId, username }));
  } else {
    // HTTP fallback for mobile/reliability
    try {
      const response = await fetch(`/lobby/challenge?token=${token}&target_id=${targetId}&username=${username}`, { method: 'POST' });
      if (!response.ok) throw new Error('HTTP challenge failed');
      console.log('Challenge sent via HTTP');
    } catch (e) {
      console.error('Challenge failed:', e);
      alert('Failed to send challenge. Please try again.'); // User feedback
    }
  }
};
