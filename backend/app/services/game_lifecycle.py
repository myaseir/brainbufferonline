...
async def finalize_match(ws, match_id, user_id, opponent_id, result_type, my_score, op_score, op_name, user_repo):
    ...
    # Note: Upstash set(..., nx=True) usually returns "OK" or True depending on version. 
    # Checking for truthiness works for both.
    if redis_client.set(lock_key, "true", nx=True, ex=30):
        ...
        # We use a pipeline to make sure all keys are set at once
        ...
        # ✅ FIXED: Upstash SDK uses .exec(), not .execute()
        pipe.exec() 
